...
; la strategia pendist prova a prendere LOW_PENALITY come
; parametro iniziale, ossia servire l'ordine con penalità massima.
; All'euristica che usiamo per cercare il tavolo da servire aggiungiamo
; un pezzo, ossia minimizzare la distanza in step in modo da completare l'ordine
; a massimo carico. (la strategia low-penality, cerca di servire quel tavolo caricando il più possibile per quel singolo ordine, poi una volta completato la fase di portare il cibo&drink al tavolo e c'è ancora roba da servire, rimette il tavolo nel pool degli ordini da evadere e ricalcola la penalità.)
; moltiplichiamo la distanza del percorso in step, per la penalità, e cerchiamo il tavolo
; con questo valore massimo;
...
=>
  ...
  (assert (exec-order (step ?current) (origin-order-step ?current) (action Inform) (table-id ?sen) (time-order ?t) (status accepted) (origin-status accepted) (drink-order ?do) (food-order ?fo) (phase 0) (fail 0) (penality (*(+ ?do ?fo)2)) (distpath 0)))
)
...
=>
  ...
  (assert (exec-order (step ?current) (origin-order-step ?current) (action Inform) (table-id ?sen) (time-order ?t) (status delayed) (origin-status delayed) (drink-order ?do) (food-order ?fo) (clean no) (phase 0) (fail 0) (penality (+ ?do ?fo)) (distpath 0)))
)
...
(defrule strategy-go-phase1
  ...
  ?f2<-(exec-order (step ?s&:(< ?s ?current)) (action Inform|Finish) (table-id ?sen) (time-order ?t) (status ?status) (priority ?p) (phase 0))
  (not (exec-order (step ?s1&:(<= ?s1 ?s)) (priority ?p1&:(>= ?p1 ?p)) (action Inform|Finish) (phase 0) (time-order ?t1&:(< ?t1 ?t))))
